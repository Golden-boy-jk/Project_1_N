...
def user_signals(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if instance.email:
            ...
            html_message = render_to_string('email/welcome_email.html', {'user': instance, 'activation_link': activation_link})
            plain_message = f"Перейдите по ссылке для активации аккаунта: {activation_link}"

            send_mail(subject, plain_message, settings.DEFAULT_FROM_EMAIL, [instance.email], html_message=html_message)
